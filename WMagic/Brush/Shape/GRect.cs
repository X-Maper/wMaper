        ...
        protected sealed override void Render()
        {
            if (!MatchUtils.IsEmpty(this.point) && !MatchUtils.IsEmpty(this.extra))
            {
                ...
                if (pen != null)
                {
                    ...
                    {
                        // 配置矩形
                        Geometry geom = (new GParse()).M(new GPoint(this.point.X - this.extra.X, this.point.Y - this.extra.Y))
                                                      .L(new GPoint[]{
                                                         new GPoint(this.point.X + this.extra.X, this.point.Y - this.extra.Y),
                                                         new GPoint(this.point.X + this.extra.X, this.point.Y + this.extra.Y),
                                                         new GPoint(this.point.X - this.extra.X, this.point.Y + this.extra.Y)
                                                      }).Z().P();
                        if (geom != null)
