...
namespace WMaper.Protocol
{
    ...
    /// Soso地图类
    /// </summary>
    public sealed class Soso : Tile
    {
        private static readonly Regex STYLE_REGEX = new Regex("^(M|S|H|T)$", RegexOptions.IgnoreCase);
        ...
        public string Style
        {
            ...
            set
            {
                if (Soso.STYLE_REGEX.IsMatch(value))
                {
        ...
        {
            int z = this.Radix + this.Start + l, y = Convert.ToInt32(Math.Pow(2, z) - r - 1);
            {
                switch (this.Style)
                {
                    case "s":
                        {
                            return "http://p" + (new Random()).Next(0, 3) + ".map.soso.com/sateTiles/" + z + "/" + (c >> 4) + "/" + (y >> 4) + "/" + c + "_" + y + ".jpg";
                        }
                    case "m":
                        {
                            return "http://p" + (new Random()).Next(0, 3) + ".map.soso.com/maptilesv2/" + z + "/" + (c >> 4) + "/" + (y >> 4) + "/" + c + "_" + y + ".png";
                        }
                    case "h":
                        {
                            return "http://p" + (new Random()).Next(0, 3) + ".map.soso.com/sateTranTiles/" + z + "/" + (c >> 4) + "/" + (y >> 4) + "/" + c + "_" + y + ".png";
                        }
                    case "t":
                        {
                            return "http://rtt" + ".map.soso.com/live/" + z + "/" + (c >> 4) + "/" + (y >> 4) + "/" + c + "_" + y + ".png";
                        }
                }
            }
            return "#";
