...
namespace WMaper.Protocol
{
    ...
    /// 高德地图类
    /// </summary>
    public sealed class Amap : Tile
    {
        private static readonly Regex THEME_REGEX = new Regex("^(ST0)$", RegexOptions.IgnoreCase);
        private static readonly Regex STYLE_REGEX = new Regex("^((S(T|H)|RD)0)$", RegexOptions.IgnoreCase);
        ...
        public string Style
        {
            ...
            set
            {
                if (Amap.STYLE_REGEX.IsMatch(value))
                {
        ...
        {
            return "http://web" + this.Style.Replace("h", "t") + ((new Random()).Next(0, 3) + 1) + ".is.autonavi.com/appmaptile?lang=zh_cn&size=1&scale=1&style=" + (Amap.THEME_REGEX.IsMatch(this.Style) ? 6 : 8) + "&x=" + c + "&y=" + r + "&z=" + (this.Radix + this.Start + l);
        }
