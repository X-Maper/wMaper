            ...
            if (!MatchUtils.IsEmpty(this.Target) && !MatchUtils.IsEmpty(this.Target.Netmap) && !MatchUtils.IsEmpty(this.Facade) && this.Target.Enable && this.Enable)
            {
                bool hide = this.Matte || !this.Viewble(this.Arise);
                {
                    if (!MatchUtils.IsEmpty(this.Handle))
                    {
                        ...
                        {
                            GOval v_oval = this.Handle as GOval;
                            {
                                v_oval.Mouse = this.Mouse;
                                v_oval.Title = this.Title;
                                v_oval.Alpha = this.Alpha;
                                v_oval.Color = this.color;
                                v_oval.Thick = this.thick;
                                v_oval.Style = this.style;
                                v_oval.Stuff = this.stuff;
                                v_oval.Slope = this.slope;
                                v_oval.Extra = this.Fit4e(this.extra);
                                v_oval.Point = this.Fit4p(this.point);
                            }
                        }
                        // 重绘图形
                    ...
                    {
                        if (!((this.Handle = this.Target.Sketch.CreateOval()).Matte = hide))
                        {
                            GOval v_oval = this.Handle as GOval;
                            {
                                v_oval.Mouse = this.Mouse;
        ...
        public void Square(Action<double> fun)
        {
            if (!MatchUtils.IsEmpty(this.Target) && !MatchUtils.IsEmpty(this.Handle) && !MatchUtils.IsEmpty(fun))
            {
                ...
                try
                {
                    ...
                    {
                        fun.Invoke(Math.Round(this.extra.X * this.extra.Y * Math.PI * 100) / 100);
                    }
                }
                catch (Exception e)
                {
